...
계정 이름, 익명 질문, 소유자 전용 계정 목록 기능을 배포할 때는 프런트엔드보다 먼저 최신 `supabase-schema.sql`을 다시 실행하세요. 기존 계정은 자동으로 1회 이름 변경 대상이 되고, 새 계정은 회원가입 때 입력한 이름으로 확정됩니다. 계정 목록 API는 서버에서도 `rices2114@gmail.com`만 허용하며 비밀번호와 인증 토큰은 반환하지 않습니다.
